...
//   }
                                ...
                                Expanded(
                                  child: Padding(
                                    ...
                                    child: Column(
                                      ...
                                      children: <Widget>[
                                        Text(getContactName(peerMobile),
                                            maxLines: 1,
  ...
  blockCall() {
    print('User ID : $userID, PEER ID : $peerID');
    blocksId.add(peerID);
    ...
    _userRef.child(userID).update({
      ...
      await Firestore.instance
        ...
        // print('REQUEST STATUS : ${documentSnapshot.data['requestStatus']}');
        if (documentSnapshot.data['idTo'] == userID) {
          await Firestore.instance
              .collection('messages')
              .document(groupChatId)
              .updateData({'requestStatus': 'block'}).then((value) {
            // setState(() {
            //   requestStatus = "block";
            // });
          });
      ...
      builder: (context, snapshot) {
        if (snapshot.hasData) {
          ...
          return snapshot.data.snapshot.value["status"].length > 0 &&
              ...
              ? snapshot.data.snapshot.value["status"] != "Off"
                          ...
                          child: ClipRRect(
                            ...
                            child: CachedNetworkImage(
                              ...
                              errorWidget: (context, url, error) => Material(
                                child: Center(child: Text("Not Avilable".tr)),
                                borderRadius: BorderRadius.all(
                                  Radius.circular(8.0),
                                ),
            ...
            await Firestore.instance
              ...
              try {
                await Firestore.instance
                    ...
                    .get()
                    .then((doc) async {
                  if (doc.data.containsKey("mute")) {
                    if (doc.data["mute"] != true) {
                      if (type == 1) {
                        ...
                        print('CALLLLLLLLLL : $peerToken');
                        sendNotification(peerToken, content, false);
                      }
                    }
                  } else {
                    if (type == 1) {
  ...
  Widget buildInput() {
    ...
    return Padding(
        padding: const EdgeInsets.only(top: 10, right: 0, bottom: 15),
        child: Material(
          elevation: 0,
          borderRadius: BorderRadius.circular(12),
          child: Container(
            ...
            decoration: BoxDecoration(
              color: Theme.of(context).brightness == Brightness.dark
                  ...
                  : Colors.white,
              // color:
              //     replyButton == true ? Colors.black : Colors.black,
              borderRadius: BorderRadius.circular(12),
            ),
            child: Padding(
                              ...
                              child: IconButton(
                                onPressed: () async {
                                  if (voiceRecording != null) {
                                    ...
                                    filename =
                                        path.basename(voiceRecording.path);
                                    final StorageReference postImageRef =
  ...
  unBlockCall() {
    ...
    _userRef.child(userID).update({
      ...
      await Firestore.instance
          ...
          .then((DocumentSnapshot documentSnapshot) async {
        // print('REQUEST STATUS : ${documentSnapshot.data['requestStatus']}');
        if (documentSnapshot.data['requestStatus'] != null) {
          await Firestore.instance
              .collection('messages')
              .document(groupChatId)
              .updateData({'requestStatus': 'accept'});
      ...
      builder: (context, snapshot) {
        if (snapshot.hasData) {
          ...
          return ListView.builder(
            ...
            itemCount: lists.length,
            itemBuilder: (BuildContext context, int index) {
              return lists[index]["castName"].contains(
                      new RegExp(forwardController.text, caseSensitive: false))
                  ? Row(
                      children: [
                        Expanded(
                          child: Column(
...
// print(path);
      ...
      setState(() {
        //isDownloading = false;
        Toast.show("Download Failed!".tr, context,
            duration: Toast.LENGTH_SHORT, gravity: Toast.BOTTOM);
      });
    }
  }
