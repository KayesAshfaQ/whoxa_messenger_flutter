...
Future getContactsFromGloble() async {
  ...
  // mobileContacts = mobileContacts.toSet().toList();

  print('NEW>>>>>>>>>>>Global>>>>>>>>>>>');
  // print(mobileContacts);
  // debugPrint('$mobileContacts');
  for (var i = 0; i < newgetContacts.length; i++) {
    // print('Number : ${mobileContacts[i]}');
    developer.log('Number', name: '${newgetContacts[i]}');
  }
...
Future<http.Response> sendNotification(
    String peerToken, String content, bool typeGroup) async {
  final response = await http.post(
    'https://fcm.googleapis.com/fcm/send',
    headers: {
      HttpHeaders.contentTypeHeader: 'application/json',
    ...
    body: jsonEncode({
      ...
      "data": {
        ...
        "user_id": userID,
        // "title": content,
        "title": typeGroup ? 'New group message.' : 'New private message',
        "message": globalName,
        "time": DateTime.now().millisecondsSinceEpoch,
        "sound": "default",
        "vibrate": "300",
      ...
      "notification": {
        "vibrate": "300",
        "priority": "high",
        "body": typeGroup ? 'New group message.' : 'New private message',
        "title": globalName,
        "sound": "default",
      }
    }),
